                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <h3>Detail Bidang Ilmu</h3>
                                            <div>
                                                <a href="{{ route('bidang_ilmu.edit', $bidang_ilmu->id) }}"
                                                    class="btn btn-warning btn-sm">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <a href="{{ route('bidang_ilmu.index') }}"
                                    ...
                                    <div class="card-body">
                                        <div class="row mb-3">
                                            <div class="col-md-4 font-weight-bold">Nama Bidang Ilmu:</div>
                                            <div class="col-md-8">{{ $bidang_ilmu->name }}</div>
                                        </div>

                                        <div class="row mb-3">
                                            <div class="col-md-4 font-weight-bold">Deskripsi:</div>
                                            <div class="col-md-8">{{ $bidang_ilmu->deskripsi }}</div>
                                        </div>

                                        <hr>

                                        <h5 class="mb-3">Penelitian Terkait</h5>
                                        @if($bidang_ilmu->penelitian->count() > 0)
                                            <ul class="list-group">
                                                @foreach($bidang_ilmu->penelitian as $p)
                                                    <li class="list-group-item">
                                                        <strong>{{ $p->judul }}</strong><br>
                                                        <small>Periode: {{ $p->mulai }} - {{ $p->akhir }}</small>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        @else
                                            <div class="alert alert-info">Belum ada penelitian yang terkait dengan bidang
                                                ilmu ini.</div>
                                        @endif
                                    </div>
                                </div>
