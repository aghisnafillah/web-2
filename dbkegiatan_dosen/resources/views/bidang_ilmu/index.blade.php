            ...
            <main>
                <div class="container-fluid px-4">
                    <div class="container mt-4">
                        <h1>Manajemen Bidang Ilmu</h1>

                        <div class="mb-3">
                            <a href="{{ route('bidang_ilmu.create') }}" class="btn btn-primary">Tambah Bidang Ilmu
                                Baru</a>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @forelse($bidang_ilmu as $index => $bidang)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $bidang->name }}</td>
                                            <td>{{ Str::limit($bidang->deskripsi, 50) }}</td>
                                            <td>
                                                <a href="{{ route('bidang_ilmu.show', $bidang->id) }}"
                                                    class="btn btn-info btn-sm" title="Detail">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('bidang_ilmu.edit', $bidang->id) }}"
                                                    class="btn btn-warning btn-sm" title="Edit">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route('bidang_ilmu.destroy', $bidang->id) }}" method="POST"
                                                    style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm" title="Hapus"
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus bidang ilmu ini?')">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center">Tidak ada data bidang ilmu</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
