                    ...
                    <div class="container mt-4">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3>{{ $dosen->gelar_depan }} {{ $dosen->name }} {{ $dosen->gelar_belakang }}</h3>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <p><strong>Jenis Kelamin:</strong>
                                            {{ $dosen->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                                        <p><strong>Tempat, Tanggal Lahir:</strong> {{ $dosen->tempat_lahir }}
                                        </p>
                                        <p><strong>Program Studi:</strong> {{ $dosen->prodi->name ?? '-' }}</p>
                                    </div>
                                    <div class="col-md-6">
                                        <p><strong>Email:</strong> {{ $dosen->email }}</p>
                                        <p><strong>Tahun Masuk:</strong> {{ $dosen->tahun_masuk }}</p>
                                ...
                                @if($dosen->kegiatan->count() > 0)
                                    <ul class="list-group">
                                        @foreach($dosen->kegiatan as $k)
                                            <li class="list-group-item">
                                                <strong>{{ $k->jenis_kegiatan->name }}</strong> -
                                                {{ $k->deskripsi }}<br>
                                                <small>{{ $k->tanggal_mulai }} s/d
                                                    {{ $k->tanggal_selesai }} di
                                                    {{ $k->tempat }}</small>
                                            </li>
                                        @endforeach
                                    </ul>
