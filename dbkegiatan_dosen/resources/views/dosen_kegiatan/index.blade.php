            ...
            <main>
                <div class="container-fluid px-4">
                    <div class="container mt-4">
                        <h1>Manajemen Dosen - Kegiatan</h1>

                        <div class="mb-3">
                            <a href="{{ route('dosen_kegiatan.create') }}" class="btn btn-primary">
                                <i class="fas fa-plus"></i> Tambah Relasi Baru
                            </a>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @forelse($dosenKegiatan as $index => $dk)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $dk->dosen->name }}</td>
                                            <td>{{ $dk->kegiatan->deskripsi }}</td>
                                            <td>{{ $dk->kegiatan->jenis_kegiatan->name }}</td>
                                            <td>
                                                {{ $dk->kegiatan->tanggal_mulai }} -
                                                {{ $dk->kegiatan->tanggal_selesai }}
                                            </td>
                                            <td>
                                                <a href="{{ route('dosen_kegiatan.show', ['dosen_id' => $dk->dosen_id, 'kegiatan_id' => $dk->kegiatan_id]) }}"
                                                    class="btn btn-info btn-sm">
                                                    <i class="fas fa-eye"></i> Lihat
                                                </a>

                                                <a href="{{ route('dosen_kegiatan.edit', ['dosen_id' => $dk->dosen_id, 'kegiatan_id' => $dk->kegiatan_id]) }}"
                                                    class="btn btn-warning btn-sm">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <form
                                                    action="{{ route('dosen_kegiatan.destroy', ['dosen_id' => $dk->dosen_id, 'kegiatan_id' => $dk->kegiatan_id]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center">Tidak ada data relasi dosen - kegiatan</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
